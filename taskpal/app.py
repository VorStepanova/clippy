...
class TaskPalApp(rumps.App):
    ...
    def __init__(self) -> None:
        ...
        self._activity_info_item = rumps.MenuItem(self._activity_info_label())
        self.menu = [
            "💬 Open Chat",
            rumps.separator,
            self._history_item,
    ...
    def _activity_label(self) -> str:
        ...
        check = "✓ " if enabled else "   "
        return f"{check}Context-aware mode"

    def _activity_info_label(self) -> str:
        enabled = self._config.get("activity_share_enabled", True)
        if enabled:
            return "   ↪ Sends app + idle time to Claude"
        return "   ↪ Only your messages are sent"
    ...
    def _toggle_activity(self, _) -> None:
        ...
        self._activity_info_item.title = self._activity_info_label()
